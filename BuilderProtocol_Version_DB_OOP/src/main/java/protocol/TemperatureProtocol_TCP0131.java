...
public class TemperatureProtocol_TCP0131 extends ThermoResistanceProtocol {

    public TemperatureProtocol_TCP0131(ReadDataBase readDataBase, ParserRTF parserRTF, int protocolScopeNumber) {
    ...
    public void calculation() {
        for (int i = 0; i < protocolPointsTable.size(); i++) { //вычисляем необходимые столбцы и добавляем в конец таблицы
            ...
            protocolPointsTable.get(i).add(calculation.calculationDmppForTemperatureTCP0131(getStandard(i)));
